  ...
  return (
    <main className="min-h-screen bg-white font-sans text-slate-900 pb-24 md:pb-0 overflow-x-hidden selection:bg-blue-100 uppercase tracking-tighter">
      ...
      <div className="fixed right-0 top-1/2 -translate-y-1/2 z-[100] hidden md:flex flex-col gap-2">
        <a href={socialLinks.whatsapp} target="_blank" className="w-12 h-12 bg-[#25D366] text-white flex items-center justify-center rounded-l-xl hover:w-16 transition-all shadow-lg"><MessageCircle size={22}/></a>
        <a href={socialLinks.facebook} target="_blank" className="w-12 h-12 bg-[#1877F2] text-white flex items-center justify-center rounded-l-xl hover:w-16 transition-all shadow-lg"><Facebook size={22}/></a>
        <a href={socialLinks.instagram} target="_blank" className="w-12 h-12 bg-black text-white flex items-center justify-center rounded-l-xl hover:w-16 transition-all shadow-lg"><Instagram size={22}/></a>
      ...
      <section id="about" className="py-16 md:py-24 bg-white px-6 scroll-mt-20">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-6">
            ...
            <p className="text-slate-600 text-lg leading-relaxed normal-case tracking-normal font-medium">Medical Line Georgia არის Eighteeth-ის ექსკლუზიური დისტრიბუტორი საქართველოში, რომელიც 10 წელზე მეტია ემსახურება ქართველ ექიმებს.</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4">
          {categories.map((cat, i) => (
            <Link href="/catalog" key={i} className={`group relative h-[200px] md:h-[300px] ${cat.color} rounded-[2rem] overflow-hidden p-5 flex flex-col justify-between shadow-sm hover:shadow-xl transition-all duration-500`}>
              ...
              <img src={cat.img} alt={cat.title} className="absolute bottom-0 right-0 w-[70%] h-auto object-contain group-hover:scale-110 transition-all duration-500" />
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 bg-slate-50 px-6 border-y border-slate-100">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="text-4xl font-black uppercase italic underline decoration-black underline-offset-8">ბლოგი</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Link href="/blog/helios-700-review" className="group bg-white p-6 rounded-[2.5rem] border border-slate-100 transition-all hover:shadow-xl">
              ...
              <p className="text-slate-500 font-bold text-sm normal-case tracking-normal">გაიგეთ, რატომ ირჩევენ ექიმები Eighteeth-ის სკანერს.</p>
            </Link>
            <Link href="/blog/rentgen-licenzireba-saqartveloshi" className="group bg-white p-6 rounded-[2.5rem] border border-slate-100 transition-all hover:shadow-xl">
              ...
              <p className="text-slate-500 font-bold text-sm normal-case tracking-normal">სრული ინფორმაცია რენტგენის კაბინეტის მოსაწყობად.</p>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section id="services" className="py-16 md:py-24 bg-white px-6 scroll-mt-20">
        <div className="max-w-7xl mx-auto mb-12">
            <h2 className="text-2xl md:text-4xl font-black uppercase tracking-tighter text-slate-900 underline decoration-blue-600 italic">სერვისი</h2>
        </div>
      ...
      {/* --- GALLERY --- */}
      <section id="gallery" className="py-16 bg-slate-900 text-white px-6 rounded-t-[3rem] -mt-10">
        <div className="max-w-7xl mx-auto mb-8 flex justify-between items-center">
            <h2 className="text-2xl md:text-4xl font-black uppercase tracking-tighter italic">გალერეა</h2>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-3 gap-3">
          {gallery.map((src, i) => (
            <div key={i} className="aspect-square rounded-2xl overflow-hidden bg-white/10 group">
              <img src={src} className="w-full h-full object-cover group-hover:scale-110 transition-all duration-700 opacity-80" alt="Gallery" />
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <div className="bg-slate-50 p-10 rounded-[3rem] shadow-sm border border-slate-100">
            ...
            <form action="https://formspree.io/f/xjgoknev" method="POST" className="space-y-4">
              <input type="text" name="name" required className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:border-blue-500 transition-all font-bold uppercase text-xs" placeholder="სახელი"/>
              <input type="text" name="phone" required className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:border-blue-500 transition-all font-bold uppercase text-xs" placeholder="ტელეფონი"/>
              <textarea name="message" rows={4} required className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:border-blue-500 transition-all font-bold uppercase text-xs" placeholder="შეტყობინება"></textarea>
              <button type="submit" className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg hover:bg-blue-700 active:scale-95 transition-all">გაგზავნა</button>
            </form>
          </div>
          <div className="h-[400px] lg:h-[500px] rounded-[3.5rem] overflow-hidden shadow-2xl border-4 border-white grayscale hover:grayscale-0 transition-all duration-700">
             <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2977.897457814324!2d44.7797825!3d41.7227488!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x4044733075b31525%3A0xc0c64e59074a383d!2zOCBEZXZpIEphYmlkemUgU3QsIFRiaWxpc2k!5e0!3m2!1sen!2sge!4v1700000000000" 
