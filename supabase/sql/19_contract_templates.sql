-- =============================================================================
-- 19_contract_templates.sql
-- Paste this in Supabase Dashboard -> SQL Editor and run once.
-- Adds selectable contract templates for product-specific sale agreements.
-- =============================================================================

ALTER TABLE public.contracts
  ADD COLUMN IF NOT EXISTS contract_template TEXT NOT NULL DEFAULT 'general',
  ADD COLUMN IF NOT EXISTS contract_body TEXT;

UPDATE public.contracts
SET contract_template = 'general'
WHERE contract_template IS NULL OR contract_template = '';

ALTER TABLE public.contracts
  DROP CONSTRAINT IF EXISTS contracts_contract_template_check;

ALTER TABLE public.contracts
  ADD CONSTRAINT contracts_contract_template_check
  CHECK (
    contract_template IN (
      'general',
      'finscan',
      'intraoral_scanner',
      'endo_equipment',
...
BEGIN
  ...
  WHERE public_token = p_public_token OR id = p_public_token
  LIMIT 1;
  ...
  SELECT
    ...
    v_contract.phone,
    v_contract.email,
    v_contract.contract_template,
    v_contract.contract_body,
